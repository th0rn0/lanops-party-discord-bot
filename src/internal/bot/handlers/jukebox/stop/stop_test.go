...
import (
	...
	"lanops/party-discord-bot/internal/bot/handlers/jukebox/stop"
...
func TestHandler_NotAdmin(t *testing.T) {
	...
	stop.Handler(newMockSession(), newMsg(false), []string{"jukebox", "stop"}, []string{}, newCfg("http://localhost"), msgCh)
	if len(msgCh) != 0 {
...
func TestHandler_Admin_APISuccess(t *testing.T) {
	...
	msgCh := make(chan channels.MsgCh, 10)
	stop.Handler(newMockSession(), newMsg(true), []string{"jukebox", "stop"}, []string{}, newCfg(srv.URL), msgCh)
	if len(msgCh) == 0 {

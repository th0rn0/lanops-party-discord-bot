...
import (
	...
	"lanops/party-discord-bot/internal/bot/handlers/jukebox/skip"
...
func TestHandler_NotAdmin(t *testing.T) {
	...
	skip.Handler(newMockSession(), newMsg(false), []string{"jukebox", "skip"}, []string{}, newCfg("http://localhost"), msgCh)
	if len(msgCh) != 0 {
...
func TestHandler_Admin_APISuccess(t *testing.T) {
	...
	msgCh := make(chan channels.MsgCh, 10)
	skip.Handler(newMockSession(), newMsg(true), []string{"jukebox", "skip"}, []string{}, newCfg(srv.URL), msgCh)
	if len(msgCh) == 0 {

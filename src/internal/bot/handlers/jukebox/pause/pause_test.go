...
import (
	...
	"lanops/party-discord-bot/internal/bot/handlers/jukebox/pause"
...
func TestHandler_NotAdmin(t *testing.T) {
	...
	pause.Handler(newMockSession(), newMsg(false), []string{"jukebox", "pause"}, []string{}, newCfg("http://localhost"), msgCh)
	if len(msgCh) != 0 {
...
func TestHandler_Admin_APISuccess(t *testing.T) {
	...
	msgCh := make(chan channels.MsgCh, 10)
	pause.Handler(newMockSession(), newMsg(true), []string{"jukebox", "pause"}, []string{}, newCfg(srv.URL), msgCh)
	if len(msgCh) == 0 {
